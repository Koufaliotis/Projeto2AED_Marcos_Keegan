...
int main(void) {
  ...
  printf("Empty set\n");
  IndicesSet* empty_set = IndicesSetCreateEmpty(12); // creates a empty set
  ...
  assert(IndicesSetGetNumElems(full_set) == 12);// return the num of elements
  assert(IndicesSetContains(full_set, 5));// get element 5
  IndicesSetDisplay(full_set);//print all elements
  printf("\n");

  printf("The complement set of the empty set\n");
  IndicesSet* comp_set = IndicesSetCreateComplement(empty_set); //makes a copy
  ...
  IndicesSet* another_set = IndicesSetCreateCopy(empty_set);//can also make a copy
  ...
  for (uint16_t e = 1; e < 13; e += 2) {
    IndicesSetAdd(another_set, e);// adds a value in the set[e] == 1 if not null
  }
  ...
  for (uint16_t e = 0; e < 7; e++) {
    IndicesSetRemove(another_set, e); //removes set[e]
  }
  ...
  IndicesSetIntersection(empty_set, full_set); //like Union but make set size smaller 
  assert(IndicesSetGetNumElems(empty_set) == 0);

  IndicesSetUnion(full_set, small_set); // combynes the sets slots so set1.slots == set2.slots
  ...
  IndicesSetDifference(another_set, small_set); //like xor dirance of sets
  IndicesSetDisplay(another_set);
  printf("\n");
