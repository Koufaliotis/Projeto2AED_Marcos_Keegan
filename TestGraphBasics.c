...
int main(void) {
  ...
  Graph* g01 = GraphCreate(6, 1, 0);//creates Graf
  GraphAddEdge(g01, 1, 2); //adds a edge ????????? in a matrix
  GraphAddEdge(g01, 1, 4);
  GraphAddEdge(g01, 3, 4);
  printf("The graph:\n");
  // Displaying in DOT format
  GraphDisplayDOT(g01); //graff display
  // Saving in DOT format
  FILE* file = fopen("simple_graph.DOT", "w");
  GraphToDOTFile(g01, file); // make a file with the graf dislay
  fclose(file);
  printf("\n");

  GraphCheckInvariants(g01);// cheques???????????

  // The lists of adjacent vertices
  for (unsigned int i = 0; i < GraphGetNumVertices(g01); i++) { //returns vertices ??? needs to be commpleted!!!!!!!
